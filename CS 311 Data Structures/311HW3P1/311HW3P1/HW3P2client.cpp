...
{ slist L;  // the list 
  ...
  try
  {
    ...
    L.replace(2, foundPos);     // replace at foundPos index (elem 8) with elem 2
    ...
    cout << "Step " << counter << endl; counter++;
    cout << "element 5 was found at index " << L.search(5) << ". 0 meaning not found" << endl;

    // 11) replace postion 7 with 10
    cout << "Step " << counter << " replace 7" << endl; counter++;
    L.replace(10, 7);       // replace the elem at pos 7 with elem 10, should throw error OutOfRange

    }//end of try
    catch(slist::OutOfRange o)
