...
Author: John Barbonio (edited based on code from Unit 8 Discussion)
Date: 6/20/2020
Desc: This function sweeps the LinkedList with an iterator and deletes the closest-to-the-front instance of the Pair with the specified passed parameters, and erases
...
void PairList<T>::deletePair(T first, T second)
{
	...
	{
		std::cout << "Found! Deleted first encountered instance (from front to back)." << std::endl;
		this->Head = p->Next;
	...
	std::cout << "Could not find a Node with that specified parameter" << std::endl;
}
/*
