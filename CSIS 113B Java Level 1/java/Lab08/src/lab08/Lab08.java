/*
John Barbonio, 3439, Lab 08: Inheritance
0518876
10/30/2019
CSIS 123B
 */
...
public class Lab08 extends Application 
{
    ...
    private Label lblLeftOutput = new Label();
    private Label lblRightOutput = new Label();
    private boolean leftCardisFlipped = false;
        ...
        lblDeckCard.setOnMouseClicked(new EventHandler<MouseEvent>()
        {
            ...
            public void handle(MouseEvent event) 
            {
                if(rightsTurn == true) 
                {
                    ...
                    lblRightCard.setGraphic(new ImageView(card3.getImage()));
                ...
                else
                {
                    ...
                    lblLeftCard.setGraphic(new ImageView(card1.getImage()));
                    leftCardisFlipped = true;
                }
                
                if(leftCardisFlipped == false) 
                {
                    lblLeftCard.setGraphic(new ImageView("file:img\\155.gif"));
                }
                
                if(card3.getValue() > card1.getValue() && card3.getValue() <= 52 && leftCardisFlipped == true) //the end conditional is to check so that one player (first play) doesn't get a free point from left's initial 55 card
                {
                    Score.addToRightScore(card3.getValue());
                    tfRightScore.setText(Integer.toString(Score.getRightScore()));
                    leftCardisFlipped = false;
                }
                else if(card1.getValue() > card3.getValue() && card1.getValue() <= 52 && leftCardisFlipped == true)
                {
    ...
    public void resetCardImages()   
    {
        ...
        lblLeftCard.setGraphic(new ImageView(card1.getImage()));
        lblDeckCard.setGraphic(new ImageView(card2.getImage()));
        lblRightCard.setGraphic(new ImageView(card3.getImage()));
    }
