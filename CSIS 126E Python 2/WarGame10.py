...
class WarGame(object):
	def __init__(self):
		...
		self.rightVal = 0
		self.leftVal = 0
		self.win = self.createWindow("War Game")
		vBox = QVBoxLayout()
	...
	def resetCardImages(self):
		try:
			# example exception in resetCardImages
			# raise CardException(inspect.currentframe().f_code.co_name)
	...
	def alert(self, strError):
		qmAlertBox = QMessageBox.warning(self.win, "Error", strError, QMessageBox.Ok)

		if(qmAlertBox == QMessageBox.Ok):
